...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (p === '/api/login' && method === 'POST') {
      rate(req, 'login', 10); const data = await parse(req), hash = scryptSync(clean(data.password, 200), admin.salt, 64)
      if (data.username !== (admin.username || 'admin') || !timingSafeEqual(hash, Buffer.from(admin.hash, 'hex'))) return json(res, 401, { error: 'Incorrect username or password.' })
      const next = randomBytes(32).toString('hex'); sessions.set(next, Date.now() + 28800000)
      res.setHeader('Set-Cookie', `bb_session=${next}; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800${process.env.NODE_ENV === 'production' ? '; Secure' : ''}`); return json(res, 200, { ok: true })
    }
    ...
    if (p.startsWith('/api/')) {
      ...
      if (p === '/api/upload' && method === 'POST') {
        ...
        const name = randomBytes(16).toString('hex') + ext; await fs.writeFile(path.join(uploads, name), b); return json(res, 201, { url: '/uploads/' + name, video: ['.mp4', '.webm'].includes(ext) })
      }
      ...
      if (p.startsWith('/api/properties/') && method === 'DELETE') { db.properties = db.properties.filter(x => x.id !== p.split('/').pop()); await save(); return json(res, 200, { ok: true }) }
      if (p === '/api/media' && method === 'DELETE') { const { url: media } = await parse(req); if (!mediaOK(media) || !media.startsWith('/uploads/')) return json(res, 400, { error: 'Only uploaded media can be deleted.' }); if (db.properties.some(x => x.images.includes(media) || x.video === media)) return json(res, 409, { error: 'Remove this media from its property and save first.' }); await fs.rm(path.join(uploads, path.basename(media)), { force: true }); return json(res, 200, { ok: true }) }
...
server.listen(Number(process.env.PORT || 3001), process.env.HOST || '127.0.0.1', () => console.log('Bayt & Brick running at http://localhost:' + (process.env.PORT || 3001)))
